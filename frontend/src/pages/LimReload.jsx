...
import { LoadingSpinner } from '../components/LoadingState.jsx'
    ...
    <section className="page lim-reload-page">
      ...
      <form className="reload-layout" onSubmit={submit}>
        <div className="reload-card">
          <div className="reload-section"><label className="reload-label" htmlFor="lim-format">LIM subject area {subjectsLoading && <LoadingSpinner label="Loading subject areas" />}</label><select id="lim-format" value={form.limFormat} onChange={event => patch('limFormat', event.target.value)} disabled={subjectsLoading || !subjectAreas.length}><option value="">{subjectsLoading ? 'Loading subject areas…' : 'Select subject area'}</option>{subjectAreas.map(subject => <option key={subject} value={subject}>{subject}</option>)}</select><small>Discovered from KUMO_TST.RAW_LIM tables named RAW_LIM_&lt;SUBJECT&gt;.</small>{subjectsError && <div className="reload-validation">Could not load subject areas: {subjectsError}</div>}</div>
