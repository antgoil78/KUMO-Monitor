...
export function createKumoEventSource(onEvent, onError, options = {}) {
  ...
  const eventTypes = ['connected', 'heartbeat', 'realtime_state', 'monitor_update', 'monitor_refresh_started', 'monitor_refresh_completed', 'workflow_run_requested', 'workflow_run_queued', 'workflow_run_status', 'workflow_run_failed']
  ...
  // SPCS ingress can delay the first streamed body chunk even after accepting
  // the SSE response. EventSource.open is the authoritative transport signal.
  source.onopen = () => onEvent?.({ type: 'connected', data: { ok: true, transport: 'eventsource' } })
